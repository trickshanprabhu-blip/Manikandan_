...
export default function CarExperience() {
  ...
  const drawFrame = useCallback((index: number) => {
    ...
    // On mobile (portrait canvas), use contain-fit so the full car is visible
    // On desktop (landscape canvas), use cover-fit for immersive experience
    const isMobilePortrait = canvasAspect < imgAspect;

    if (isMobilePortrait) {
      // Contain-fit: show the entire image, no cropping
      drawW = canvas.width;
      drawH = canvas.width / imgAspect;
      drawX = 0;
      // Position car in upper-center area (40% from top) instead of dead center
      drawY = (canvas.height - drawH) * 0.4;
    } else {
      // Cover-fit for landscape/desktop: fill the canvas
      drawW = canvas.width;
      drawH = canvas.width / imgAspect;
      drawX = 0;
  ...
  const overlayTexts = [
    { text: "Engineering Excellence", startFrame: 0, endFrame: 23 },
    { text: "Precision Craftsmanship", startFrame: 36, endFrame: 63 },
    { text: "Every Component Matters", startFrame: 76, endFrame: 103 },
    { text: "Built to Perfection", startFrame: 106, endFrame: 120 },
  ];
